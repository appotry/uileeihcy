...
// 		m_pEraseBtn->SetConfigHeight(16);
		...
		m_pEraseBtn->SetDrawFocusType(BUTTON_RENDER_DRAW_FOCUS_TYPE_NONE);
		m_pEraseBtn->SetConfigLayoutFlags(LAYOUT_ITEM_ALIGN_CENTER);
...
// 			continue;
		...
		//if (bAnimate)
		{
            UI::IIntTimeline* pTimeline = (UI::IIntTimeline*)pAnimateMgr->CreateTimelineInstance(UI::E_TIMELINE_INT);
			pTimeline->SetFromTo(pItem->GetHeight(), nItemLastHeight);
			pTimeline->SetDuration(150);
            int* pnHeight = NULL;
            pItem->GetHeightRef(&pnHeight);
            pTimeline->SetOutRef(pnHeight);

            UI::IStoryboard*  pStoryboard = pAnimateMgr->CreateStoryboard();
            pStoryboard->SetUserData((LPARAM)pItem);
            pStoryboard->SetNotifyObj(static_cast<UI::IMessage*>(m_pIQQLoginAccountListBox));
            pStoryboard->SetID((int)_ttoi(pItem->GetAccount()));
			pStoryboard->AddTimeline(pTimeline);
            pStoryboard->Begin();
		}
	}
}
...
void  QQLoginAccountListBox::OnEraseBtnClick()
{
    ...
    m_pIQQLoginAccountListBox->RemoveItem(m_pIQQLoginAccountListBox->GetHoverItem(), LISTITEM_OPFLAG_NOALL);
